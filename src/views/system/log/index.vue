...
import { getAddLogList } from "@/api/system/log";
...
const dictData = ref({
    ...
    sys_opt_type: [{label: '残疾人', value: '1'},{label: '黑名单', value: '2'}]
})
//查询表单
...
//查询 列表数据
const getList = () => {
    getAddLogList(proxy.addDateRange(queryParams.value, dateRange.value,"myself")).then((res) => {
...
const handleQuery = () => {
    ...
    console.log(sys_authentication)
    getList()
}
