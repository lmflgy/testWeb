					...
					<el-row class="row-mar">
						...
						<el-col :span="8" class="row">
							<el-form-item label="客户名称：" prop="name" :rules="[{ required: true, message: '请输入客户名称' }]">
								<el-input v-model="form.name" clearable :disabled="type == 1 ?false : true" />
							</el-form-item>
						</el-col>
						<el-col :span="8" class="row">
							<el-form-item label="证件类型：" prop="certType"
								:rules="[{ required: true, message: '请选择证件类型' }]">
								<el-select v-model="form.certType" clearable :disabled="type == 1 ?false : true">
						...
						<el-col :span="8" class="row">
							<el-form-item label="证件号码：" prop="certNo" :rules="[{ required: true, message: '请输入证件号码' }]">
								<el-input v-model="form.certNo" clearable maxlength="18"
...
<script setup name="BlackListEdit">
	...
	const {
		sys_authentication,
		cert_type
	} = proxy.useDict("sys_authentication", "cert_type");
	...
	const handleManagerPwd = async (boo, pwd) => {
		if (pwd) {
			...
			let title = type.value == 1 ? '申请' : '修改'
			let res = {}
			if (type.value == 2)  res = await updateBlackUser(obj)
			else  res = await addBlackUser(obj)
			if (res.code == 200) {
	...
	// let title = ''
	// debugger
	// if(route.query.type == 1) title="黑名单信息新增"
	// if(route.query.type == 2) title="黑名单信息修改"
	// if(route.query.type == 3) title="黑名单信息详情"
	...
	//关闭当前页面
	const cancelBack = ()=>{
		const view = {path:'/customer/blackListEdit',name: "BlackListEdit"}
